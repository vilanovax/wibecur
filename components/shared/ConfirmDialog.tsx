...
export default function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        className="relative w-full max-w-sm bg-white rounded-2xl shadow-xl border border-wibe p-5 text-right"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <p className="text-wibe-secondary text-sm mb-6">{message}</p>
        <div className="flex gap-2 justify-end">
          <button
            type="button"
            onClick={onCancel}
            disabled={loading}
            className="px-4 py-2.5 rounded-xl border border-wibe text-foreground text-sm font-medium hover:bg-wibe-surface disabled:opacity-50"
          >
            {cancelLabel}
          </button>
          <button
