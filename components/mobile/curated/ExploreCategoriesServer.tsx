...
type ExploreCategoriesServerProps = {
  categories: CuratedCategory[];
};

export default function ExploreCategoriesServer({ categories }: ExploreCategoriesServerProps) {
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 lg:grid-cols-6 lg:gap-3">
        {displayCats.map((cat) => {
          ...
          const cover = resolveNextImageSrc(getCategoryCoverUrl(slug));
          return (
            <Link
              key={cat.id}
              href={`/categories/${slug}`}
              className="group relative aspect-[4/5] overflow-hidden rounded-xl border border-wibe bg-wibe-card shadow-sm transition-transform active:scale-[0.98] lg:aspect-[5/4] lg:hover:border-primary/25"
            >
              {cover ? (
                <Image
                  src={cover.src}
      ...
      <Link
        href="/categories"
        className="mt-3 flex w-full items-center justify-center rounded-xl border border-primary/25 bg-primary/[0.04] py-2.5 wibe-caption font-semibold text-primary transition-colors hover:bg-primary/10 active:scale-[0.99] lg:mt-3.5 lg:inline-flex lg:w-auto lg:px-6"
      >
        مشاهده همه دسته‌ها
      </Link>
    </section>
