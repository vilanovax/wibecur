/**
 * Inner lists skeleton — used inside page Suspense (Header/BottomNav already mounted).
 */
export default function ListsPageContentSkeleton() {
  return (
    <div className="w-full min-w-0 space-y-0 pb-6 lg:pb-4" aria-hidden>
      ...
      <div className="sticky top-14 z-20 border-b border-wibe bg-wibe-card/95 backdrop-blur-md">
        <div className="flex gap-1.5 overflow-hidden px-3 py-2 lg:px-0">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-8 w-20 shrink-0 animate-pulse rounded-full bg-wibe-surface"
            />
          ))}
        </div>
      ...
      <div className="mt-2 w-full min-w-0 max-lg:px-3 lg:mt-4 lg:px-0">
        <section className="mb-3 lg:mb-5">
          ...
          <div className="aspect-[2/1] min-h-[108px] w-full animate-pulse rounded-xl bg-wibe-surface sm:aspect-[5/3] sm:min-h-[128px] lg:aspect-[3/2] lg:min-h-[180px]" />
        </section>

        <section className="mb-4 lg:mb-6">
          <div className="mb-3 h-6 w-36 animate-pulse rounded bg-wibe-surface lg:mb-4" />
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-[repeat(auto-fill,minmax(220px,1fr))] lg:gap-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
