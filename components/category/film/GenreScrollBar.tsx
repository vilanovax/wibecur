      ...
      >
        {chips.map((genre) => (
          <Link
            ...
            className="flex shrink-0 snap-start items-center gap-1.5 rounded-full border border-wibe bg-wibe-surface px-3.5 py-2 text-sm font-medium text-foreground transition-all hover:bg-wibe-surface lg:hover:border-primary/30 lg:hover:bg-primary/5"
          >
            <span>{genre.icon}</span>
            <span>{genre.label}</span>
            {genre.listCount > 0 && (
              <span className="rounded-full bg-white/90 px-1.5 py-0.5 wibe-caption font-bold text-wibe-secondary tabular-nums">
