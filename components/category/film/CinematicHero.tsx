    ...
    <section className="relative mx-4 mt-4 overflow-hidden rounded-lg">
      <div className="relative aspect-[16/9] min-h-[220px]">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-4 pb-5">
          <h1 className="wibe-h1 text-white">{category.icon} {category.name}</h1>
          <p className="wibe-small text-white/90 mt-1 max-w-xl line-clamp-2">
            {category.description || 'بهترین لیست‌های سینمایی این هفته'}
          </p>

          <div className="flex flex-wrap gap-2 mt-3 wibe-caption text-white/95">
            <span className="bg-white/10 backdrop-blur-sm px-2.5 py-1 rounded-md">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <Link
              ...
              className="inline-flex items-center justify-center py-3 px-5 rounded-md wibe-small font-semibold text-white bg-primary hover:bg-primary-dark transition-colors"
            >
              کشف ترندها
            </Link>
            <Link
              href={`/lists?category=${category.slug}&create=1`}
              className="inline-flex items-center justify-center py-3 px-5 rounded-md wibe-small font-semibold text-white border border-white/50 hover:bg-white/10 transition-colors"
            >
              ساخت لیست
            </Link>
          </div>
