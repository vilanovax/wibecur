...
import { listCafeCoverItems } from '@/lib/cafe-cover-items';
import { checkObjectStorageReady } from '@/lib/object-storage-readiness';

/** GET /api/admin/items/cafe-cover-items?listId= | ?categoryId= */
  ...
  try {
    ...
    if (listId) {
      ...
      const [items, storage] = await dbQuery(() =>
        Promise.all([
          listCafeCoverItems(prisma, { listId }),
    ...
    const [items, storage] = await dbQuery(() =>
      Promise.all([
        listCafeCoverItems(prisma, { categoryId: categoryId! }),
        checkObjectStorageReady(),
      ])
    );
    ...
    console.error('cafe-cover-items error:', error);
    return NextResponse.json(
