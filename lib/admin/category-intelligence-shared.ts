/**
 * Pure category-intelligence helpers safe for client bundles.
 * Keep Prisma / DB I/O out of this module (bundle-barrel / server split).
 */
...
export function computeCategoryListMetricsFromAggregate(
  ...
  const activeListsPercent =
    listCount > 0 ? (activeListCount / listCount) * 100 : 0;
...
export function computeCategoryListMetrics(
  ...
  for (const l of lists) {
    ...
    if (l.saveCount > 0) activeLists += 1;
  }
  const engagementRatio = totalViews > 0 ? (totalSaves / totalViews) * 100 : 0;
  const activeListsPercent =
...
export const EMPTY_GROWTH: CategorySaveGrowthStats = {
  percent: 0,
  recent: 0,
  previous: 0,
};

export function computeSaveGrowthPercent(
  recent: number,
  previous: number
): number {
  if (previous > 0) {
...
export function isMonetizableCategory(
  row: Pick<CategoryIntelligenceRow, 'listCount' | 'engagementRatio'>
): boolean {
  return row.listCount >= 2 || row.engagementRatio >= 2;
}

export function buildCategoryInsightLine(
  ...
  const withSignal = active.filter(
    (r) => r.saveGrowthRecent > 0 || r.listCount > 0
  );
  ...
  const top = [...withSignal].sort((a, b) => {
    const scoreA =
      a.saveGrowthRecent > 0
        ? a.saveGrowthRecent * 10 + a.engagementRatio
        : a.engagementRatio;
    const scoreB =
      b.saveGrowthRecent > 0
        ? b.saveGrowthRecent * 10 + b.engagementRatio
        : b.engagementRatio;
...
export function getCategoryBadgeFlags(row: CategoryIntelligenceRow) {
  ...
  const isNewActivity =
    row.saveGrowthPrevious === 0 && row.saveGrowthRecent > 0;
  const isFastRising =
    isNewActivity || growth > 10 || row.avgSavesPerList >= 50;
  return {
    isFastRising,
    isLowEngagement:
      engagement < 10 && row.listCount > 0 && !isFastRising,
...
export function buildCategoryPulse(
  ...
  const fastest = [...activeRows].sort(
    (a, b) => b.saveGrowthPercent - a.saveGrowthPercent
  )[0];
  const avgSaveGrowth =
    rows.length > 0
      ? Math.round(
          (rows.reduce((s, r) => s + r.saveGrowthPercent, 0) / rows.length) *
            10
        ) / 10
      : 0;
