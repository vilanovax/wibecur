# WibeCur — مرجع فنی پروژه (آخرین بروزرسانی: 2026-03-26)
...
## 22. تغییرات اخیر (2026-03-25)

### 22.1 ثبت‌نام با موبایل
- **صفحه**: `/register` — فرم ساده با موبایل + رمز عبور + نام نمایشی
- **API**: `POST /api/auth/register` — ایجاد کاربر با شماره موبایل (unique)، آواتار پیش‌فرض `vibe`
- **فیلد جدید**: `phone` (String, unique) در مدل `users`
- **لاگین**: هم با موبایل و هم ایمیل ساپورت میشه (تب‌سوئیچ در `/login`)
- **Auth config**: `lib/auth-config.ts` — ورود با `phone` یا `email`

### 22.2 ساده‌سازی پروفایل (MVP)
- **حذف شده از نمایش**: `ProfileStats`، `ProfileRankCard`، `ProfileTopLists`، `ProfileLevel`، `ProfileAchievements`
- **باقی‌مانده**: `ProfileHeader` (آواتار + نام + ویرایش + **دکمه خروج**) + `ProfileTabs` (لیست‌ها/ذخیره‌ها/فعالیت‌ها)
- **آیکون ذخیره‌ها**: از `Bookmark` به `Star` تغییر کرد (یکسان با لیست)

### 22.3 صفحه آیتم — بازچیدمان
**ترتیب جدید بخش‌ها:**
1. Hero (تصویر + عنوان + متا + **دکمه گزارش**)
2. Social Proof
3. **درباره این {فیلم/کتاب/...}** + **اطلاعات تکمیلی** (یکپارچه با جداکننده گرادیانی)
4. لینک خارجی
5. لیست‌ها
6. نظرات
7. Similar Items / Also Liked / Trending
8. (گزارش از پایین صفحه به hero منتقل شد)

**تغییرات کلیدی:**
- عنوان «درباره این آیتم» → «درباره این فیلم/کتاب/...» بر اساس `categorySlugToLabel` mapping
- بخش «درباره» و «اطلاعات تکمیلی» در یک کارت ادغام شدند
- `ItemReportButton` حالا `variant="icon"` دارد (استایل شیشه‌ای hero)
- دکمه «پاسخ» از کامنت‌های آیتم حذف شد

### 22.4 صفحه لیست — ساده‌سازی MVP
- **حذف شده**: بخش وایرال (درصد + بنر)، دکمه‌های بزرگ تکراری «ذخیره» و «اشتراک‌گذاری»
- **تغییر متن**: «افزودن آیتم» → «پیشنهاد آیتم»
- CTA‌های اصلی فقط در هدر (ستاره + share)

### 22.5 Placeholder لوکال
- `lib/placeholder-liara.ts`: به جای picsum.photos (فیلترشده در ایران) → SVG گرادیانی لوکال تولید و در MinIO آپلود میشه
- `app/api/placeholder/route.ts`: SVG inline برمی‌گردونه (بدون نیاز به سرویس خارجی)
- ۱۰ ترکیب رنگی گرادیان تعریف شده

### 22.6 سیستم کامنت — Pagination
- **فیلدهای جدید** در `comment_settings`: `initialDisplayCount` (پیش‌فرض: 3)، `loadMoreCount` (پیش‌فرض: 10)
- **CommentSection**: اول ۳ کامنت نمایش، بعد دکمه «نمایش بیشتر (X نظر دیگه)»
- **Admin UI**: دو فیلد جدید در تنظیمات کامنت‌ها
- **API**: مقادیر pagination در response کامنت‌ها برگردانده میشه

### 22.7 بوکمارک و ذخیره — رفع باگ‌ها
- **BookmarkButton** (لیست): همیشه `fetchBookmarkStatus` صدا میزنه → ستاره درست toggle میشه
- **ItemSaveButton** (آیتم): رفتار toggle — اگه ذخیره نیست → مودال باز میشه، اگه ذخیره هست → از همه لیست‌ها حذف میشه
- **saved-status API**: فیلد `savedItemId` اضافه شد (ID آیتم کپی‌شده در لیست کاربر، برای DELETE صحیح)
- **SaveToPersonalListModal**: بعد از افزودن موفق → ۶۰۰ms delay → auto-close
- **SaveToPersonalListModal**: دیگه `/api/admin/settings` صدا نمیزنه (باعث 401 برای کاربر عادی میشد)
